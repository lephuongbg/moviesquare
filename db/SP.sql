...
s1.`room_id` AS `10:00`, s2.`room_id` AS `11:30`, s3.`room_id` AS `14:00`, s4.`room_id` AS `16:45`, s5.`room_id` AS `20:00`, s6.`room_id` AS `21:30` FROM `movies` AS m
...
ON m.`id` = s3.`movie_id` AND s3.`show_time` LIKE '%14:00%'
LEFT JOIN `shows` AS s4
ON m.`id` = s4.`movie_id` AND s4.`show_time` LIKE '%16:45%'
LEFT JOIN `shows` AS s5
ON m.`id` = s5.`movie_id` AND s5.`show_time` LIKE '%20:00%'
LEFT JOIN `shows` AS s6
ON m.`id` = s6.`movie_id` AND s6.`show_time` LIKE '%21:30%'

WHERE m.`class` LIKE '%movie_now_showing%' AND m.`id` = movie_id
GROUP BY DATE(s0.`show_time`);
END //
DELIMITER ;
